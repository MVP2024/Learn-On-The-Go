...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ('user', 'payment_type', 'get_content_title', 'amount', 'status', 'created_at', 'completed_at')
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj and obj.status == 'completed':
            ...
            return self.readonly_fields + ('user', 'payment_type', 'discipline', 'lesson', 'amount', 'status')
        return self.readonly_fields


@admin.register(PurchasedContent)
class PurchasedContentAdmin(admin.ModelAdmin):
    list_display = ('user', 'get_content_type', 'get_content_title', 'get_payment_amount', 'purchased_at')
    list_filter = ('purchased_at', 'payment__payment_type')
    search_fields = ('user__email', 'user__first_name', 'user__last_name',
                    'discipline__title', 'lesson__title')
...
class PriceConfigurationAdmin(admin.ModelAdmin):
    list_display = ('get_content_title', 'get_content_type', 'price', 'is_free',
                   'discount_price', 'discount_end_date', 'get_current_price_display')
    ...
    fieldsets = (
        ('Контент', {
            'fields': ('discipline', 'lesson')
        }),
        ('Цена', {
            'fields': ('price', 'is_free', 'discount_price', 'discount_end_date', 'get_current_price_display')
        }),
        ('Временные метки', {
