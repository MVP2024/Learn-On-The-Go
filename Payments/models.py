...
class Payment(models.Model):
    ...
    payment_type = models.CharField(
        max_length=20, choices=PAYMENT_TYPE_CHOICES, verbose_name="Тип платежа"
    )
    ...
    payment_method = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Способ оплаты"
    )
    transaction_id = models.CharField(
        max_length=255, unique=True, blank=True, null=True, verbose_name="ID транзакции"
    )
    yookassa_payment_id = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="ID платежа в ЮKassa"
    )
    yookassa_confirmation_url = models.URLField(
        blank=True, null=True, verbose_name="URL для оплаты в ЮKassa"
    )
    # Для Stripe храним только идентификатор intent'а для аудита — client_secret НЕ хранится
    stripe_payment_intent_id = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Stripe PaymentIntent ID"
    )
    ...
    def clean(self):
        ...
        if self.payment_type == "discipline" and not self.discipline:
            raise ValidationError(
                'Для типа платежа "дисциплина" необходимо указать дисциплину'
            )
...
class PriceConfiguration(models.Model):
    ...
    discount_end_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата окончания скидки"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    ...
    def get_current_price(self):
        ...
        if (
            ...
            and timezone.now() <= self.discount_end_date
        ):
