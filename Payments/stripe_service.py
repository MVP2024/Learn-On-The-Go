"""
Простой сервис для работы со Stripe.
...
class StripeService:
    ...
    def create_payment_intent(self, amount: Decimal, description: str, transaction_id: str = None) -> dict:
        """
        ...
        try:
            ...
            intent = stripe.PaymentIntent.create(
                ...
                metadata={
                    'transaction_id': transaction_id or 'auto-generated'
                },
    ...
    def get_payment_info(self, payment_intent_id: str) -> dict:
        """
        ...
        try:
            ...
            return {
                'payment_intent_id': intent.id,
                'status': intent.status,
                'amount': Decimal(intent.amount) / 100,  # Конвертируем обратно из центов
            ...
            logger.error(f"Ошибка получения информации о платеже {payment_intent_id}: {str(e)}")
    ...
    def create_refund(self, payment_intent_id: str, amount: Decimal = None, reason: str = None) -> dict:
        """
        ...
        try:
            ...
            logger.error(f"Ошибка создания возврата для платежа {payment_intent_id}: {str(e)}")
            raise Exception(f"Не удалось создать возврат: {str(e)}")

    @staticmethod
    def validate_webhook_signature(payload: bytes, sig_header: str, endpoint_secret: str) -> bool:
        """
